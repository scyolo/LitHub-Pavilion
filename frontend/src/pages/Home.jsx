...
export default function Home() {
  ...
  const latest = useQuery({ queryKey: ["papers", "latest", scope], queryFn: ({ signal }) => api.papers({ ...scope, size: 5, sort: "created_desc" }, signal), refetchInterval: 30000 });
  ...
  return <div className="dashboard-page page-enter">
    <section className="page-heading dashboard-heading">
      ...
      <div className="heading-actions"><span className="muted small">本地快照 · {data ? formatDate(data.generated_at, true) : "读取中"}</span><button className="button secondary small-button" onClick={() => { dashboard.refetch(); latest.refetch(); }} disabled={dashboard.isFetching} aria-label="刷新总览数据"><Icon name="refresh" size={15} className={dashboard.isFetching ? "spin" : ""} />刷新</button></div>
    </section>
    ...
    {dashboard.isPending ? <LoadingState rows={3} /> : dashboard.error ? <ErrorState message={dashboard.error.message} onRetry={() => dashboard.refetch()} /> : <>
      <section className="metrics-grid" aria-label="论文库概览">
        <Metric icon="book" label="已收录论文" value={number(data.total)} note="当前筛选范围的本地记录" color="blue" />
