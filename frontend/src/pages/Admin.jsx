...
export default function Admin() {
  return api.isSnapshot ? <SnapshotAdmin /> : <LiveAdmin />;
}
...
function LiveAdmin() {
  ...
  return <div className="admin-page page-enter">
    ...
    {statusQuery.error ? <ErrorState message={statusQuery.error.message} onRetry={() => statusQuery.refetch()} /> : <div className="admin-summary">
      <section className="panel run-panel"><SectionTitle icon="refresh" title="采集状态"><span className={`run-state ${status?.running ? "running" : "idle"}`}><i />{status?.running ? "正在同步" : "当前空闲"}</span></SectionTitle><h3>{status?.running ? status.current || "准备采集任务…" : "让系统为你持续发现研究"}</h3><p>{status?.running ? `任务 ${status.run_id || "准备中"}` : status?.schedule?.next_crawl_at ? "每周采集一次；图表刷新只查询本地数据，不额外请求论文源。" : "定时采集未启用；可手动同步，或通过配置启用启动采集。"}</p>{status?.running && <div className="run-progress"><div role="progressbar" aria-label="采集进度" aria-valuenow={Math.round(progress)} aria-valuemin={0} aria-valuemax={100}><span style={{ width: `${progress}%` }} /></div><small>{status.progress || "等待上游响应"}</small></div>}<div className="run-actions"><button className="button primary" onClick={() => start.mutate({ scope: "weekly" })} disabled={busy}><Icon name="refresh" size={16} className={start.isPending ? "spin" : ""} />立即同步元数据</button><button className="button secondary" onClick={() => setConfirmBackfill(!confirmBackfill)} disabled={busy}>回填 2023–2025</button></div>{confirmBackfill && <div className="confirm-box"><p>历史回填会请求外部 API，按已完成单元续传。确认启动？</p><button className="button secondary small-button" disabled={busy} onClick={() => { setConfirmBackfill(false); start.mutate({ scope: "backfill", years: [2023, 2024, 2025] }); }}>确认回填</button><button className="text-button" onClick={() => setConfirmBackfill(false)}>取消</button></div>}{notice && <p className="action-notice" role="status">{notice}</p>}{status?.last_error && <p className="action-error" role="alert">最近错误：{status.last_error}</p>}</section>
      <section className="panel schedule-panel"><SectionTitle icon="calendar" title="更新计划" /><dl><div><dt><span className="status-dot" />论文元数据</dt><dd>{status?.schedule?.next_crawl_at ? formatDate(status.schedule.next_crawl_at, true) : "未启用定时采集"}<small>下次采集 · {status?.schedule?.timezone || "Asia/Shanghai"}</small></dd></div><div><dt><Icon name="link" size={14} />开放链接补全</dt><dd>{status?.schedule?.next_links_at ? formatDate(status.schedule.next_links_at, true) : "未启用定时维护"}<small>只更新链接，不存储文件</small></dd></div></dl><div className="schedule-note"><Icon name="shield" size={17} /><p>链接模式已启用<br /><span>官方 / OA 跳转，不下载 PDF</span></p></div></section>
    </div>}
    {status?.site_sync && <section className="panel schedule-panel"><SectionTitle icon="link" title="静态快照与网站发布" /><dl><div><dt>本地快照</dt><dd>{SNAPSHOT_STATES[status.site_sync.snapshot_status] || status.site_sync.snapshot_status}<small>{status.site_sync.generated_at ? `${formatDate(status.site_sync.generated_at, true)} · ${number(status.site_sync.paper_count)} 篇` : "尚无导出记录"}</small></dd></div><div><dt>GitHub Pages</dt><dd>{PUBLISH_STATES[status.site_sync.publication_status] || status.site_sync.publication_status}<small>{status.site_sync.message || "采集完成后自动导出；只有确认线上版本一致才显示已上线。"}</small></dd></div><div><dt>启动自动采集</dt><dd>{status.site_sync.startup_crawl_enabled ? "已启用" : "未启用"}</dd></div></dl></section>}
